...
start(Tokenizer) ->
    ...
    T2 = erlmd_tokenizer:set_state(T1, heading_atx_prefix, 0),
    ...
    case erlmd_tokenizer:current(T2) of
        ...
        _ ->
            %% Not a heading
            T3 = erlmd_tokenizer:exit(T2, heading_atx),
            {nok, T3}
    end.
...
after_prefix(Tokenizer) ->
    Prefix = erlmd_tokenizer:get_state(Tokenizer, heading_atx_prefix),
        ...
        C when C =:= $\t orelse C =:= $\s ->
            if
                PrefixCount < 3 ->
                    ...
                    T2 = erlmd_tokenizer:set_state(T1, heading_atx_prefix, PrefixCount + 1),
...
before(Tokenizer) ->
    case erlmd_tokenizer:current(Tokenizer) of
        $# ->
            ...
            T3 = erlmd_tokenizer:set_state(T2, heading_atx_count, 1),
            {{next, heading_atx_sequence_open}, T3};
        _ ->
            %% Not a heading
            T1 = erlmd_tokenizer:exit(Tokenizer, heading_atx),
            {nok, T1}
    end.

%% @doc Count opening '#' sequence (1-6 marks).
-spec sequence_open(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
sequence_open(Tokenizer) ->
    Count = erlmd_tokenizer:get_state(Tokenizer, heading_atx_count),

    case erlmd_tokenizer:current(Tokenizer) of
        $# when Count < ?HEADING_ATX_OPENING_FENCE_SIZE_MAX ->
            %% More hash marks and haven't hit limit
            T1 = erlmd_tokenizer:consume(Tokenizer),
            T2 = erlmd_tokenizer:set_state(T1, heading_atx_count, Count + 1),
            {{next, heading_atx_sequence_open}, T2};

        C when C =:= eof; C =:= $\n ->
            %% Empty heading at EOF or newline
            T1 = erlmd_tokenizer:exit(Tokenizer, heading_atx_sequence),
            T2 = erlmd_tokenizer:exit(T1, heading_atx),
            {ok, T2};
...
-spec after_sequence(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
...
-spec content_inside(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
