...
start(Tokenizer) ->
    ...
    T2 = erlmd_tokenizer:set_state(T1, thematic_break_count, 0),

    %% Handle optional leading whitespace (up to 3 spaces)
    case erlmd_tokenizer:current(T2) of
        C when C =:= $\t; C =:= $\s ->
...
-spec after_prefix(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
after_prefix(Tokenizer) ->
    Prefix = erlmd_tokenizer:get_state(Tokenizer, thematic_break_prefix),
        ...
        C when C =:= $\t orelse C =:= $\s ->
            if
                PrefixCount < 3 ->
                    ...
                    T2 = erlmd_tokenizer:set_state(T1, thematic_break_prefix, PrefixCount + 1),
...
%% @private
-spec before(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
before(Tokenizer) ->
    case erlmd_tokenizer:current(Tokenizer) of
        C when C =:= $*; C =:= $-; C =:= $_ ->
            %% Valid marker - store it and start sequence
            T1 = erlmd_tokenizer:set_state(Tokenizer, thematic_break_marker, C),
            T2 = erlmd_tokenizer:enter(T1, thematic_break_sequence),
            {{retry, thematic_break_inside}, T2};
        _ ->
            %% Not a thematic break marker
            T1 = erlmd_tokenizer:exit(Tokenizer, thematic_break),
            {nok, T1}
    end.
...
-spec inside(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
inside(Tokenizer) ->
    Marker = erlmd_tokenizer:get_state(Tokenizer, thematic_break_marker),
    Count = erlmd_tokenizer:get_state(Tokenizer, thematic_break_count),

    case erlmd_tokenizer:current(Tokenizer) of
        M when M =:= Marker ->
            %% Same marker - consume and continue counting
            T1 = erlmd_tokenizer:consume(Tokenizer),
            T2 = erlmd_tokenizer:set_state(T1, thematic_break_count, Count + 1),
            {{next, thematic_break_inside}, T2};

        C when C =:= $\t; C =:= $\s ->
            %% Whitespace - exit sequence and continue at break
            T1 = erlmd_tokenizer:exit(Tokenizer, thematic_break_sequence),
        ...
        _ ->
            ...
            T1 = erlmd_tokenizer:exit(Tokenizer, thematic_break_sequence),
            T2 = erlmd_tokenizer:exit(T1, thematic_break),
            {nok, T2}
    end.
...
%% Exported for state machine.
-spec consume_whitespace(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
...
%% @private
-spec finish(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
finish(Tokenizer) ->
    Count = erlmd_tokenizer:get_state(Tokenizer, thematic_break_count),
