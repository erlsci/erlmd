...
start(T) ->
    case erlmd_tokeniser:current(T) of
        $` ->
            T1 = erlmd_tokeniser:enter(T, code_text),
            T2 = erlmd_tokeniser:enter(T1, code_text_sequence),
            %% Initialize size counter for sequence length
            T3 = erlmd_tokeniser:set_state(T2, code_text_open_size, 0),
...
sequence_open(T) ->
    Size = erlmd_tokeniser:get_state(T, code_text_open_size),

    case erlmd_tokeniser:current(T) of
        $` ->
            T1 = erlmd_tokeniser:consume(T),
            T2 = erlmd_tokeniser:set_state(T1, code_text_open_size, Size + 1),
            {{next, code_text_sequence_open}, T2};
        _ ->
            %% End of opening sequence
            T1 = erlmd_tokeniser:exit(T, code_text_sequence),
            %% Initialize close counter
            T2 = erlmd_tokeniser:set_state(T1, code_text_close_size, 0),
...
between(T) ->
    case erlmd_tokeniser:current(T) of
        $` ->
            ...
            T1 = erlmd_tokeniser:enter(T, code_text_sequence),
            T2 = erlmd_tokeniser:set_state(T1, code_text_close_size, 0),
        ...
        _ ->
            %% Regular data
            T1 = erlmd_tokeniser:enter(T, code_text_data),
...
data(T) ->
    case erlmd_tokeniser:current(T) of
        $` ->
            ...
            T1 = erlmd_tokeniser:exit(T, code_text_data),
...
sequence_close(T) ->
    OpenSize = erlmd_tokeniser:get_state(T, code_text_open_size),
    CloseSize = erlmd_tokeniser:get_state(T, code_text_close_size),

    case erlmd_tokeniser:current(T) of
        $` when CloseSize < OpenSize ->
            %% Still counting closing sequence
            T1 = erlmd_tokeniser:consume(T),
            T2 = erlmd_tokeniser:set_state(T1, code_text_close_size, CloseSize + 1),
        ...
        _ when CloseSize =:= OpenSize ->
            ...
            T1 = erlmd_tokeniser:exit(T, code_text_sequence),
            T2 = erlmd_tokeniser:exit(T1, code_text),
            {ok, T2};
...
reclassify_and_fail(T) ->
    ...
    T1 = erlmd_tokeniser:set_events(T, NewEvents),
    {nok, T1}.
...
reclassify_sequence_as_data(T) ->
    ...
    erlmd_tokeniser:set_events(T, NewEvents).
