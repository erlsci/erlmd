...
start(Tokenizer) ->
    case erlmd_tokenizer:current(Tokenizer) of
        ...
        $\s ->
            ...
            T1 = erlmd_tokenizer:enter(Tokenizer, code_indented),
            T2 = erlmd_tokenizer:set_state(T1, code_indent_spaces, 0),
...
after_prefix(Tokenizer) ->
    Spaces = erlmd_tokenizer:get_state(Tokenizer, code_indent_spaces),
    ...
    case erlmd_tokenizer:current(Tokenizer) of
        $\s when SpaceCount < ?CODE_INDENT_SIZE ->
            ...
            T2 = erlmd_tokenizer:set_state(T1, code_indent_spaces, SpaceCount + 1),
        ...
        _ ->
            ...
            T1 = erlmd_tokenizer:exit(Tokenizer, code_indented),
            {nok, T1}
    end.

%% @doc Begin a line of code content.
-spec begin_line(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
...
content_inside(Tokenizer) ->
    case erlmd_tokenizer:current(Tokenizer) of
        eof ->
            %% End of code at EOF
            T1 = erlmd_tokenizer:exit(Tokenizer, code_flow_chunk),
            T2 = erlmd_tokenizer:exit(T1, code_indented),
            {ok, T2};
...
-spec further_start(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
further_start(Tokenizer) ->
    case erlmd_tokenizer:current(Tokenizer) of
        ...
        $\s ->
            ...
            T1 = erlmd_tokenizer:set_state(Tokenizer, code_indent_spaces, 0),
...
-spec further_end(erlmd_tokenizer:tokenizer()) ->
    {erlmd_tokenizer:state_result(), erlmd_tokenizer:tokenizer()}.
