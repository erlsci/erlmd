...
start(T) ->
    ...
    T1 = erlmd_tokeniser:enter(T, document),
    %% Try optional BOM
    case erlmd_tokeniser:attempt_construct(T1, bom, nok) of
        {ok, T2} ->
            parse_flow_loop(T2);
        {nok, T2} ->
            parse_flow_loop(T2)
...
-spec parse_flow_loop(erlmd_tokeniser:tokenizer()) ->
    {erlmd_tokeniser:state_result(), erlmd_tokeniser:tokenizer()}.
...
parse_flow_loop(T) ->
    case erlmd_tokeniser:current(T) of
        eof ->
            ...
            T1 = erlmd_tokeniser:exit(T, document),
            {ok, T1};
        _ ->
            ...
            case erlmd_state:call(flow, T) of
                ...
                {nok, T1} ->
                    ...
                    T2 = erlmd_tokeniser:exit(T1, document),
                    {error, {unexpected_content, erlmd_tokeniser:current(T1)}, T2}
            end
    end.
